...
def get_thresholds(multiset, p):
    low = low_threshold(multiset, p*100)
    high = high_threshold(multiset, p*100)
...
def get_hlf_thresholds(eval_all, p):
    ...
    for hlf in all_hlf_values.keys():
        ...
        if f_type == 'delay':
            ...
            window_deltas = [entry[0] for entry in this_hlf_values if entry[1] > 0]
            low, high = get_thresholds(window_deltas, p_hard_set)
            # rint(high)
            delayed_bundle_sizes = [entry[1] for entry in this_hlf_values if entry[0] > high]
            _, bundle_thresh_for_delay = get_thresholds(delayed_bundle_sizes, p)
        else:
            low, high = get_thresholds(this_hlf_values, p)
        all_hlf_thresh[hlf] = (low, high)

    return all_hlf_thresh, bundle_thresh_for_delay
...
def hle_theta_by_hlf(event_dict, traffic_type, theta, eval_complete, instance_hlf_complete, comp_type_dict,
                     all_hlf_thresholds, frequencies_last, id_counter, last_case_set_dic, bundle_thresh_for_delay):
...
def get_type_thresholds(eval_all, comp_type_dict, p):
    ...
    for type_comp_pair in all_types_values.keys():
        ...
        if f_type == 'delay':
            ...
            window_deltas = [entry[0] for entry in type_comp_values if entry[1] > 0]
            low, high = get_thresholds(window_deltas, p_hard_set)
            delayed_bundle_sizes = [entry[1] for entry in type_comp_values if entry[0] > high]
            _, bundle_thresh_for_delay = get_thresholds(delayed_bundle_sizes, p)
            low, high = get_thresholds(window_deltas, p_hard_set)
        else:
    ...
    return all_types_thresh, bundle_thresh_for_delay


def get_case_set(instance_list, event_dict):
    case_list = []
    for entry in instance_list:
        if isinstance(entry, int) or len(entry) == 1:
...
def hle_theta_by_type(event_dict, traffic_type, theta, eval_complete, instance_hlf_complete, comp_type_dict,
                      all_types_thresholds, frequencies_last, id_counter, last_case_set_dic, bundle_thresh_for_delay):
    ...
    for hlf in eval_at_theta.keys():
        ...
        if class_v in traffic_type:
            if f_type == 'delay':
                no_windows = hlf_value[0]
                no_instances = hlf_value[1]
                if no_instances > bundle_thresh_for_delay:  # only considering bundles that are big enough
                    hle = {'f-type': f_type, 'entity': entity, 'class': class_v, 'value': (no_windows, no_instances),
                           'component': entity_comp, 'theta': theta}
                else:
                    continue
...
def hle_all(event_dict, traffic_type, eval_complete, instance_hlf_complete, comp_type_dict, p, type_based):
    ...
    if type_based:
        type_thresholds, bundle_thresh_for_delay = get_type_thresholds(eval_complete, comp_type_dict, p)
        for theta in eval_complete.keys():
        ...
        hlf_thresholds, bundle_thresh_for_delay = get_hlf_thresholds(eval_complete, p)
        for theta in eval_complete.keys():
